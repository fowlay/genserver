...
public final class PxCb extends CallBackBase {
    ...
    private GenServer getPortsender(Side side) {
        for (; true; ) {
            ...
            if (gs != null) {
                ...
                final String key =
                        side == Side.UE ? "access-UDP" : side == Side.SP ? "core-UDP" : "badkey";
    ...
    public CastResult handleCast(Object message) {
        ...
        if (mb instanceof KeepAliveMessage) {
            ...
            if (side == Side.SP) {
                Util.seq(Mode.KEEP_ALIVE, Side.PX, Util.Direction.NONE,
                        "drop keep-alive response");
            }
            else {
            ...
            try {
                ...
                if (type == Type.REQUEST) {
                    ...
                    if (method == Method.REGISTER ||
                        ...
                        if (!hh.isEmpty()) {
                            ...
                            final String addrPort =
                                    topRoute.substring(indexOfSipColon+4, indexOfSemi);
                    ...
                    if (otherSide == Side.UE) {
                        ...
                        if (sa == null) {
                            // can happen if the targeted UE is not registered
                            Util.seq(Mode.SIP, Side.PX, Direction.NONE,
                                    "not registered: %s, dropping %s request",
                                    toUser,
                                    method.toString());
                        } else {
                            final byte[] addr =
                                    ((InetSocketAddress) sa).getAddress().getAddress();
                            final Integer port =
                                    Integer.valueOf(((InetSocketAddress) sa).getPort());
                        ...
                        Util.seq(Mode.SIPDEBUG, side, Direction.NONE,
                                "gsForward thread name is: %s", gsForward.getThread().getName());
                        gsForward.cast(ism.setDestination(addr, port));
                    }
                    ...
                    // response .. easy, just drop topmost via and use new top via as destination
                    ...
                    if (Main.RECORD_ROUTE && ism.side == Side.UE &&
                            (method == Method.INVITE || method == Method.SUBSCRIBE)) {
                        final String rrHeaderField = String.format("<sip:%s:%s;lr>",
                                toDottedAddress(Main.sipAddrUe),
                                Main.sipPortUe.intValue());
                    ...
                    if (sm.getMethod() == Method.REGISTER) {
                        if (sm.isSuccess()) {
                            ...
                            if (regAttempt != null && regAttempt.booleanValue()) {
                                ...
                                registry.put(
                                    toUser,
                                    UdpCb.createSocketAddress(ismOut.destAddr, ismOut.destPort));
                                Util.seq(Mode.DEBUG, Side.PX, Direction.NONE,
                                        String.format("registered: %s -> %s:%d",
                                                toUser,
                                                Util.bytesToIpAddress(ismOut.destAddr),
                                                ismOut.destPort));
